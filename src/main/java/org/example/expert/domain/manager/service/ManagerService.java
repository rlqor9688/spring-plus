...
//        System.out.println(" todo 조회 성공");

        if (todo.getUser() == null || !ObjectUtils.nullSafeEquals(user.getId(), todo.getUser().getId())) {
            throw new InvalidRequestException("담당자를 등록하려고 하는 유저가 유효하지 않거나, 일정을 만든 유저가 아닙니다.");
        }

//        System.out.println(" manager 조회 시도");
        User managerUser = userRepository.findById(managerSaveRequest.getManagerUserId())
                .orElseThrow(() -> new InvalidRequestException("등록하려고 하는 담당자 유저가 존재하지 않습니다."));
//        System.out.println(" manager 조회 성공");

        System.out.println("userId: " + user.getId());
        System.out.println("managerId: " + managerUser.getId());
...
//        System.out.println(" 검증 통과");
        Log logs = new Log(user.getId(), null, todo.getId(), "담당자 등록 요청됨");

        System.out.println(" logService.saveLog 호출 직전");
